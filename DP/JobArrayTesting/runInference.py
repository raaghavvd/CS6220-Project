...
TEST_DF = pd.read_csv("ml-1m2.test.csv").drop(["Timestamp"], axis=1)
TRAIN_DF = pd.read_csv("ml-1m2.train.csv").drop(["Timestamp"], axis=1)
...
TEST_UIDS = TEST_DF["UserID"].unique().tolist()
TRAIN_UIDS = TRAIN_DF["UserID"].unique().tolist()

for _id in TEST_UIDS:
    print(_id, _id in TRAIN_UIDS)
...
def GetAverageMovRating(movid):
    if not movid in MOVIE_AVERAGES:
        avg = TRAIN_DF[TRAIN_DF["MovieID"] == movid]["Rating"].mean()
        MOVIE_AVERAGES[movid] = avg
        return avg
    return MOVIE_AVERAGES[movid]
